...
Tensor Tensor::add(const Tensor& rhs) const {
    if (shape_ != rhs.shape_) {
        throw std::invalid_argument("Tensor add requires equal shapes");
    }
...
Tensor Tensor::matmul(const Tensor& rhs) const {
    if (rank() != 2 || rhs.rank() != 2) {
        throw std::invalid_argument("Tensor matmul currently supports rank-2 tensors only");
    }
...
Tensor Tensor::relu() const {
    Tensor out(shape_);
    std::transform(data_.begin(), data_.end(), out.data_.begin(), [](float value) {
...
Tensor Tensor::softmax() const {
    ...
    const auto max_it = std::max_element(data_.begin(), data_.end());
    ...
    if (sum == 0.0F || !std::isfinite(sum)) {
        throw std::runtime_error("Tensor softmax normalization failed");
    }
