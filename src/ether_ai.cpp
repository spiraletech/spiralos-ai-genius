...
std::string context_for(const HostDescriptor& host) {
    ...
    switch (host.kind) {
        case HostKind::StandaloneWindows:
            return "You are Spiral Ether AI running as the standalone native Windows assistant. The same organic runtime can be embedded into EtherPlay, Hakui, EtherBeat, and other Spiral hosts.";
...
std::string Runtime::send(std::string_view text) {
    ...
    const std::string reply = shell_.chat(visible);
